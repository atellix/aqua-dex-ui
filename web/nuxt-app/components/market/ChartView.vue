...
export default {
    ...
    setup(props, context) {
        ...
        var chartCurrent = [ chartStart[0] ]
        var chartData = ref([ { data: chartCurrent } ])

        var repeat
        repeat = (j, vr) => {
            if (j < chartStart.length) {
                chartData.value[0].data.push(chartStart[j])
                console.log('Updated chart')
                console.log(JSON.stringify(chartData.value[0].data))
                console.log(vr)
                vr.apexChart.updateSeries(chartData.value)
                setTimeout((i, v) => {
                    repeat(i + 1, v)
                }, 1000, j, vr)
            }
